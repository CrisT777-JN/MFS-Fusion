...
class L_TV(torch.nn.Module):
    ...
    def forward(self, IA, IB, IF):
        ...
        TV_LossA = tvAH + tvAW
        TV_LossB = tvBH + tvBW
        TV_Loss = 10 * TV_LossA + 30 * TV_LossB
        # TV_Loss=50*torch.max(TV_LossA,TV_LossB)
        return TV_Loss
...
class L_Intensity(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused):
        intensity_joint_max = torch.max(image_A, image_B)
        # Loss_intensity = F.l1_loss(image_fused, intensity_joint_max)
        Loss_intensity = F.l1_loss(image_fused, intensity_joint_max) #+ 0.3 * F.l1_loss(image_fused, (image_A+image_B)/2.0)
...
class fusion_loss_vif(nn.Module):
    ...
    def forward(self, image_A, image_B, image_fused):
        ...
        l = self.l(image_A, image_B, image_fused)
        ...
        fusion_loss = loss_l1 + loss_gradient + l + loss_SSIM
