...
class Cross_WindowAttention(nn.Module):#cross
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
基于窗口的多头自我注意（W-MSA）模块，具有相对位置偏差。
它支持移位和非移位窗口。
    Args:
        dim (int): Number of input channels.
        window_size (tuple[int]): The height and width of the window.
...
class Cross_SwinTransformerBlock(nn.Module):#cross_block
    r""" Swin Transformer Block.
...
class Cross_BasicLayer(nn.Module):#cross_basic
    """ A basic Swin Transformer layer for one stage.
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        for i_layer in range(self.Ex_num_layers):
            layer = RSTB(dim=embed_dim,
                         ...
                         depth=Ex_depths[i_layer],#这Ex_depths=[4],除了第一个位置存在，其他位置都不存在，那他遍历个毛线
        ...
        # build the last conv layer in deep feature extraction#深度特征提取中最后一个conv层的构建
        if resi_connection == '1conv':
...
if __name__ == '__main__':
    ...
    x1 = torch.randn((1, 1, height, width))
    x = model(x,x1)
    print(x.shape)
