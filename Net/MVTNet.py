...
class MyNet(nn.Module):
    def __init__(self, in_chans=1, hidden_chans=[32,64, 96, 128], out_chans=1, linear=True):
        ...
        self.p_x = nn.Conv2d(in_chans, 3, 3, 1, 1)
        self.p_y = nn.Conv2d(in_chans, 3, 3, 1, 1)

        self.pre_x = nn.Conv2d(3, hidden_chans[0], 3, 1, 1)
        self.pre_y = nn.Conv2d(3, hidden_chans[0], 3, 1, 1)


        self.un_x = mobile_vit_small()
        self.un_y = mobile_vit_small()

        self.fuse0 = MEF(dim=hidden_chans[0])
        self.fuse1 = MEF(dim=hidden_chans[1])
        self.fuse2 = MEF(dim=hidden_chans[2])
        self.fuse3 = MEF(dim=hidden_chans[3])
        ...
        self.dec = Decode(hidden_chans[0],hidden_chans[1],hidden_chans[2],hidden_chans[3])
        self.__load_pre()
    def forward(self, x,y):
        ...
        x = self.p_x(x)
        y = self.p_y(y)
        short_x = self.pre_x(x)
        short_y = self.pre_y(y)


        x = self.un_x.conv_1(x)
        x = self.un_x.layer_1(x)
        y = self.un_y.conv_1(y)
        y = self.un_y.layer_1(y)
        x, y, out = self.fuse0(x, y)
        fuses.append(out)
        x = self.un_x.layer_2(x)
        y = self.un_y.layer_2(y)
        x, y, out = self.fuse1(x, y)
        fuses.append(out)
        x = self.un_x.layer_3(x)
        y = self.un_y.layer_3(y)
        x, y, out = self.fuse2(x, y)
        fuses.append(out)
        x = self.un_x.layer_4(x)
        y = self.un_y.layer_4(y)
        x, y, out = self.fuse3(x, y)

        fuses.append(out)


        out = self.dec(fuses[0],fuses[1],fuses[2],fuses[3])
    ...
    def __load_pre(self):
        pre_model="./Net/mobilevit_s.pht"
        self.un_x.load_state_dict(torch.load(pre_model))
        print(f"loading pre_model ${pre_model}")

        self.un_y.load_state_dict(torch.load(pre_model))
        print(f"loading pre_model ${pre_model}")
...
class Decode(nn.Module):
    ...
    def forward(self,x1,x2,x3,x4):
        ...
        up3 = self.conv_up3(torch.cat((up4,x3),1))
